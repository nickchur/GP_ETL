CREATE FUNCTION s_grnplm_vd_hr_edp_srv_wf.pr_rebuild_org_structure() 
	RETURNS text
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
--		q_query text;
	...
	begin		
		...
		wf_name = 'GPDDS_PR_REBUILD_ORGSRTUCTURE';
		log_id = s_grnplm_vd_hr_edp_srv_wf.pr_Log_start('GPDDS_PR_REBUILD_ORGSRTUCTURE');   --Log
...
-- 		, query_txt text, block int, query_num int);
		...
		-- Определяем шаблон запроса для формирования таблицы
			query = ' insert into s_grnplm_vd_hr_edp_dds.nfbk_org_tree'
...
--			values(format(query, v_cal_date), 1, cur_iter_0);
		...
		-- Формируем шаблон для создания временных таблиц
			query = 'drop table if exists lvl_%1$s;'
					...
					|| E'\n\t, cal_date'
					|| E'\n\t, lvl'
					|| E'\nfrom s_grnplm_vd_hr_edp_dds.nfbk_org_tree'
					|| E'\nwhere 1 = 1'
					|| E'\n\tand lvl = %1$s'
					|| E'\n)'
		...
		-- Определяем максимальный уровень включенности
			max_lvl = (select max(lvl) from s_grnplm_vd_hr_edp_dds.nfbk_org_tree);
...
--				query = 'insert into dnf_org_tree'
	...
	close cur2;
            ...
            step_id = 'insert into  s_grnplm_vd_hr_edp_dds.dnf_org_tree';
			...
			insert into s_grnplm_vd_hr_edp_dds.dnf_org_tree
            ...
            SELECT
                current_date, cal_date, t.org_id, org_id_lvl
                ...
                , lvl_15_org_id, lvl_15_date_start, lvl_15_date_end
                , lvl_16_org_id, lvl_16_date_start, lvl_16_date_end
            FROM s_grnplm_vd_hr_edp_dds.dnf_org_tree t;
			
			-- логирование воркфлоу GPDDS_REBUILD_MOVEMENT_DETAIL
			perform s_grnplm_vd_hr_edp_srv_wf.pr_Log_end(log_id, 's_grnplm_vd_hr_edp_dia.dnf_org_tree','cal_date','cal_date',null); --Log
			return 'OK';
