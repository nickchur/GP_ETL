...
| Файл | Описание |
|------|----------|
| `pr_swf_start.sql` | Запуск super workflow — точка входа для старта группы ETL-задач — [описание алгоритма](pr_swf_start.md) |
| `pr_swf_start_all.sql` | Массовый запуск всех воркфлоу в группе |
| `pr_swf_start_ctl.sql` | Запуск воркфлоу с контролем выполнения и обработкой ошибок — [описание алгоритма](pr_swf_start_ctl.md) |
...
| `pr_swf_wf_group_replace.sql` | Замена/обновление группы воркфлоу — [описание алгоритма](pr_swf_wf_group_replace.md) |
...
| `pr_std_sql.sql` | Генератор диагностических/сервисных SQL-запросов по коду `action` (не выполняет, а возвращает текст) — [описание алгоритма](pr_std_sql.md) |
