CREATE FUNCTION s_grnplm_vd_hr_edp_srv_wf.pr_dia2hist(wf text, key_date text DEFAULT NULL::text, not_hsh text[] DEFAULT '{}'::text[]) 
	RETURNS text
	LANGUAGE plpgsql
	VOLATILE
as $body$
...
--        exe = format('truncate s_grnplm_vd_hr_edp_dia.tmp_%s_hist', wf);
        ...
        db_key = pg_get_table_distributedby(format('s_grnplm_vd_hr_edp_dia.dia_%s', wf)::regclass::oid);
        ...
        for rec in (select a.attnum num, a.attname fld, format_type(a.atttypid, a.atttypmod) flf
                    ...
                    and a.attrelid = format('s_grnplm_vd_hr_edp_dia.dia_%s', wf)::regclass::oid  order by a.attnum) loop
...
--            if rec.fld not in ('load_date', 'actual_date', 'ctl_validfrom', 'ctl_loading', 'ctl_action') then
        ...
        exe = format($sql$
            ...
            on commit drop as
                select %4$s as mod_date, %3$s as hash
                    %2$s 
                from s_grnplm_vd_hr_edp_dia.dia_%1$s a
            distributed by (%5$s)
    ...
    exception when OTHERS then
        ...
        begin
            ...
            perform s_grnplm_vd_hr_edp_srv_wf.pr_Log_error(log_id, sqlerrm, e_detail, e_hint, e_context);
